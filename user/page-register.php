...
if (!empty($_POST['register_reg'])) {
    ...
    if ($sanitized_user_login == '') {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Please enter username.", "sakura") ./*请输入用户名。*/
            '<br />';
    } elseif (!validate_username($sanitized_user_login)) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Invalid characters, please enter a valid username.", "sakura") ./*此用户名包含无效字符，请输入有效的用户名。*/
            '<br />';
        $sanitized_user_login = '';
    } elseif (username_exists($sanitized_user_login)) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("This username has been registered.", "sakura") ./*该用户名已被注册。*/
            '<br />';
    }
    ...
    if ($user_email == '') {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Please enter email address.", "sakura") ./*请填写电子邮件地址。*/
            '<br />';
    } elseif (!is_email($user_email)) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Invalid email address.", "sakura") ./*电子邮件地址不正确。*/
            '<br />';
        $user_email = '';
    } elseif (email_exists($user_email)) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("This email address has been registered.", "sakura") ./*该电子邮件地址已经被注册。*/
            '<br />';
    }
    ...
    if (strlen($_POST['user_pass']) < 6) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Password length is at least 6 digits.", "sakura") ./*密码长度至少6位。*/
            '<br />';
    } elseif ($_POST['user_pass'] != $_POST['user_pass2']) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Inconsistent password entered twice.", "sakura") ./*两次输入的密码不一致。*/
            '<br />';
    }
    ...
    if (akina_option('login_validate') && strlen($_POST['verification']) > 0) {
        $error .= '<strong>' . __("Error", "sakura") ./*错误*/
            '</strong>：' . __("Please drag the slider to verify identity", "sakura") ./*请拖动滑块验证身份*/
            '<br />';
    }
    ...
    if ($error == '') {
        ...
        if (!$user_id) {
            $error .= '<strong>' . __("Error", "sakura") ./*错误*/
                '</strong>：' . __("Unable to complete registration request...Please contact", "sakura") ./*无法完成注册请求... 请联系*/
                '<a href=\"mailto:' . get_option('admin_email') . '\">' . __("administrator", "sakura") ./*管理员*/
                '</a>！<br />';
            ...
            <div class="ex-register">
                ...
                <form action="<?php echo $_SERVER["REQUEST_URI"]; ?>" method="post">
                    <p><input type="text" name="user_login" tabindex="1" id="user_login" class="input"
                              ...
                              placeholder="用户名" required/></p>
                    <p><input type="text" name="user_email" tabindex="2" id="user_email" class="input"
                              value="<?php if (!empty($user_email)) echo $user_email; ?>"
                              size="25" placeholder="电子邮箱" required/></p>
                    ...
                    <p><input class="input" tabindex="4" type="text" tabindex="21" size="25" placeholder="手机号"
                              required/></p>
                    <p><input class="input" tabindex="4" type="text" tabindex="21" size="22" placeholder="验证码"
                              required/></p>
                    <br>
                    <button>获取验证码</button>
                        ...
                        <div id="verification-slider">
                            <div id="slider">
                                ...
                                <span id="label">»</span><span
                                        id="labelTip"><?php _e("Drag the slider to verify", "sakura")/*拖动滑块验证*/ ?></span>
                            </div>
                    ...
                    <input class="button register-button" name="submit" type="submit"
                           value="<?php _e("Sign up", "sakura")/*注 册*/ ?>">
                </form>
            </div>
        <?php }else{
